...
import json
from ui import get_social_links_html, get_social_links_data, get_market_data, get_community_data, get_cg_summary_data
import streamlit.components.v1 as components
from utils import get_description, load_coingecko_data, get_repo_data
...
repo_link_choice = links_dict["github"] if isinstance(links_dict["github"], list) else list(links_dict["github"])
repo_choice = st.selectbox("Select a repo", repo_link_choice)
input_type = st.radio(
            "Filter Repo Commits", ("Filter commits by Date", "Filter commits by n", "Don't filter commits (large repos make take time)"))
    
if input_type == "Filter commits by Date":
    filter_option = st.date_input("Pulling repository commits from: (default 2021-01-01)", datetime.date(2021, 1, 1))
    d = datetime.datetime.combine(filter_option, datetime.datetime.min.time())
elif input_type == "Filter commits by n":
    filter_option = st.number_input("Pull the first `n` commits",min_value=1, max_value=None, value=100)
elif input_type == "Filter commits by n":
    filter_option = None
...
with git_container:
    ...
    with st.form(key='my_form_to_submit'):
        ...
        if submit_button:
            if input_type == "Filter commits by Date":
                commit_history = get_repo_data(repo_choice, since = d) #d.strftime("%Y-%m-%d")
            elif input_type == "Filter commits by n":
                commit_history = get_repo_data(repo_choice, first_n_commits = filter_option)
