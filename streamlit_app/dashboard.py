...
def body(commit_history, git_container):
    ...
    with git_container.container():
        ...
        markdownn = ' | '.join([f"{stat[1][1]} **{stat[0]}**: {stat[1][0]}" for stat in repo_stats.items()])
        git_container.markdown(markdownn,  unsafe_allow_html=True)
        try:
            git_container.subheader(f"Contributor: {contributor_stats.pop('Contributor')[0]}")
    ...
    with git_container.expander("Contributors Overview", expanded=True):
        git_container.write(ui.plot_top_contributors(utils.get_top_contributors(commit_history)))
